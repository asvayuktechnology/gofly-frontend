...
import DestinationSelect, { DestinationOption } from "@/Components/Common/UI/Selects/DestinationSelects";
import SiteBtn from "@/Components/Common/SiteBtn/SiteBtn";
import { HOTEL_CATEGORIES, type HotelCategoryValue } from "@/utils";

// Centralized from utils/index.ts – single source of truth for property types
const propertyTypeOptions = HOTEL_CATEGORIES as unknown as { label: string; value: HotelCategoryValue }[];

const HotelTabContent = () => {
  const router = useRouter();
  const [destination, setDestination] = useState<DestinationOption | null>(null);
  const [selectedPropertyType, setSelectedPropertyType] = useState<{ label: string; value: string } | null>(null);

  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    const params = new URLSearchParams();
    if (destination?.value) params.set("destination", destination.value);
    if (selectedPropertyType?.value) params.set("propertyType", selectedPropertyType.value);
  ...
  return (
    <form className="filter-input show" onSubmit={handleSearch}>
      ...
      <div className="single-search-box">
        <div className="custom-select-dropdown destination-dropdown">
          <select
            ...
            onChange={(e) => {
              const selected = propertyTypeOptions.find((cat) => cat.value === e.target.value);
              setSelectedPropertyType(selected || null);
            }}
            className="absolute inset-0 opacity-0 cursor-pointer z-10"
          >
            <option value="">Select Property Type</option>
            {propertyTypeOptions.map((cat) => (
          ...
          <div className="input-field-value">
            <div className="destination">
              <h6>{selectedPropertyType?.label || "Select Property Type"}</h6>
      ...
      <SiteBtn
        ...
        text="SEARCH"
        disabled={!destination && !selectedPropertyType}
        svgIcon={
