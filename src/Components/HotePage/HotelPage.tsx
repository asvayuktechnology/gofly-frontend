...
import HotelSidebar from "@/Components/HotePage/HotelSidebar";
...
import { useHotel } from "@/services/hotelService";
...
export default function HotelPage({ id }: HotelPageProps) {
    // ─── Dynamic hotel fetch ──────────────────────────────
    ...
    const toggle = (faqId: string) => {
        setActive((prev) => (prev === faqId ? "" : faqId));
    };

    // ─── Derived dynamic values from hotel ────────────────
    const hotelName = (hotel as any)?.name || "Rosewood Hotel & Resorts";
    const formatLocation = (loc: any): string | null => {
        if (!loc) return null;
        if (typeof loc === "string") return loc;
        if (typeof loc === "object") {
            // Backend HotelLocation: { address, city, state, country, postalCode, latitude, longitude }
    ...
    const hotelLocation =
        formatLocation((hotel as any)?.location) ||
        ((hotel as any)?.city && (hotel as any)?.country
            ? `${(hotel as any).city}, ${(hotel as any).country}`
            : formatLocation((hotel as any)?.destination) ||
              (hotel as any)?.city ||
              (hotel as any)?.state ||
              (typeof (hotel as any)?.destination === "string" ? (hotel as any).destination : null) ||
              (hotel as any)?.destination?.name ||
              "Himachal Pradesh, India");
    const hotelRating = (hotel as any)?.rating ?? (hotel as any)?.starRating ?? 5;
    const hotelReviewCount = (hotel as any)?.totalReviews ? String((hotel as any).totalReviews) : "23,046";
    const hotelDescription =
        (hotel as any)?.description ||
        (hotel as any)?.overview ||
        (hotel as any)?.about ||
        "";
    const hotelMapUrl =
        (hotel as any)?.mapEmbedUrl ||
        (hotel as any)?.mapUrl ||
        (hotel as any)?.locationUrl ||
        "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3649.5647631857846!2d90.36311167605992!3d23.83407118555764!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3755c14c8682a473%3A0xa6c74743d52adb88!2sEgens%20Lab!5e0!3m2!1sen!2sbd!4v1700138349574!5m2!1sen!2sbd";
    // ─── Image URL helper (TypeError fix: uploads/* is relative, need BASE_URL) ──
    const IMG_BASE = (UPLOAD_URL as string) || (BASE_URL as string) || "";
    ...
    // Gallery: prefer bannerImages / galleryImages / image from API, fallback to static
    const toGalleryFormat = (imgs: string[] | undefined, prefix = "hotel") =>
        imgs && imgs.length > 0
            ? imgs.map((img, idx) => ({ id: `${prefix}-${idx}`, image: getImageUrl(img) }))
            : null;

    const dynamicSliderImages =
        toGalleryFormat((hotel as any)?.bannerImages) ||
        toGalleryFormat((hotel as any)?.sliderImages) ||
        toGalleryFormat((hotel as any)?.images) ||
        null;

    const dynamicGalleryImages =
        toGalleryFormat((hotel as any)?.galleryImages) ||
        ((hotel as any)?.image ? [{ id: "gallery-main", image: getImageUrl((hotel as any).image) }] : null) ||
        null;

    const fallbackSliderImages = [
        { id: "1", image: "/assets/img/hotel-dt-gallery-img2.webp" },
        { id: "2", image: "/assets/img/hotel-dt-gallery-img7.webp" },
        { id: "3", image: "/assets/img/hotel-dt-gallery-img2.webp" },
    ];
    const fallbackGalleryImages = [
        { id: "4", image: "/assets/img/hotel-dt-gallery-img2.webp" },
        { id: "5", image: "/assets/img/hotel-dt-gallery-img7.webp" },
        { id: "6", image: "/assets/img/hotel-dt-gallery-img2.webp" },
    ];

    const sliderImages = dynamicSliderImages || fallbackSliderImages;
    const galleryImages = dynamicGalleryImages || fallbackGalleryImages;

    // ─── Dates & Availability: rooms dynamic from API ─────────
    const dynamicRooms =
        (hotel as any)?.rooms && Array.isArray((hotel as any).rooms) && (hotel as any).rooms.length > 0
            ? (hotel as any).rooms.map((r: any, idx: number) => ({
                  id: r._id || String(idx + 1),
                  category: r.category || r.name || `Room ${idx + 1}`,
                  name: r.name || r.category || `Room ${idx + 1}`,
                  price: r.pricePerNight ?? r.price ?? 0,
                  oldPrice: r.originalPricePerNight ?? r.oldPrice,
                  description: r.description || "",
                  // TypeError fix: r.images may be undefined, ensure array & fix relative URLs
                  images: Array.isArray(r.images) && r.images.length > 0
                      ? r.images.map((img: string) => getImageUrl(img))
                      : r.image
                        ? [getImageUrl(r.image)]
                        : [],
                  services: Array.isArray(r.includedServices)
                      ? r.includedServices
                      : Array.isArray(r.services)
                        ? r.services
                        : [],
                  currency: r.currency || (hotel as any)?.currency || "INR",
    ...
    // ─── FAQs & AdditionalInfo dynamic ────────────────────────
    const dynamicFaqs =
        (hotel as any)?.faqs && Array.isArray((hotel as any).faqs) && (hotel as any).faqs.length > 0
            ? (hotel as any).faqs
            : null;
    const displayFaqs = dynamicFaqs || FAQ_ITEMS;

    const dynamicAdditionalInfo =
        (hotel as any)?.additionalInfo && Array.isArray((hotel as any).additionalInfo) && (hotel as any).additionalInfo.length > 0
            ? (hotel as any).additionalInfo
            : null;

    // ─── Loading / Error states (hook updates dynamically when id changes) ───
    if (!id) {
        return <div className="py-20 text-center text-red-500">Invalid Hotel ID — please provide a valid id prop.</div>;
    }
    if (isLoading) {
        return <div className="py-20 text-center">Loading hotel details...</div>;
    }
    if (isError) {
        return (
            <div className="py-20 text-center text-red-500">
                Failed to load hotel details. {(error as Error)?.message || "Please try again."}
            </div>
        );
    }
    if (!hotel) {
    ...
    return (
        <>
            <HotelBreadcrumb hotelName={hotelName} location={hotelLocation} rating={Number(hotelRating)} reviewCount={String(hotelReviewCount)} mapUrl={hotelMapUrl} />

            <HotelGallery
                sliderImages={sliderImages}
                galleryImages={galleryImages}
                videoThumbnail={(hotel as any)?.videoThumbnail || "/images/video-thumb.jpg"}
                videoUrl={(hotel as any)?.videoUrl || "https://www.youtube.com/watch?v=u31qwQUeGuM"}
            />
                ...
                <div className="container mx-auto ">
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                                ...
                                <div className="package-info-wrap mb-60">
                                    <div className="mb-16 px-3">
                                        <div className="hotel-dt-description-area mb-50">
                                            ...
                                            {hotelDescription ? (
                                                <p className="whitespace-pre-line">{hotelDescription}</p>
                                            ) : (
                                                <>
                                                    <p>
                                                        {hotelName} is a premium hospitality property located in {hotelLocation}.
                                                        Experience refined hospitality, personalized service, and unique sense of place.
                                                    </p>
                                                    <p>
                                                        Guests enjoy world-class amenities, including dining, wellness, curated cultural
                                                        experiences, and unparalleled hospitality. Price per night starts at {hotel.currency || "₹"}
                                                        {hotel.pricePerNight} {hotel.originalPricePerNight ? `(Original: ${hotel.currency || "₹"}${hotel.originalPricePerNight})` : ""}.
                                                        {hotel.cancellationPolicy ? ` Cancellation: ${hotel.cancellationPolicy}.` : ""}
                                                    </p>
                                                </>
                                            )}
                                            {/* Always show fallback if no description to keep UI rich */}
                                            {!hotelDescription && (
                                                <p>
                                        ...
                                        <div className="hotel-dt-facilities-area mb-50">
                                            ...
                                            {(hotel as any)?.amenities && Array.isArray((hotel as any).amenities) && (hotel as any).amenities.length > 0 ? (
                                                <div className="facilities-wrap">
                                                    {(hotel as any).amenities.map((group: any, gIdx: number) => {
                                                        const cat = typeof group === "string" ? group : group.category || `Group ${gIdx + 1}`;
                                                        const items = typeof group === "string" ? [] : group.items || [];
                                                        return (
                                                            <div key={gIdx} className="single-facilities">
                                                                <h6>{cat}</h6>
                                                                <ul className="facilities-list">
                                                                    {(items.length > 0 ? items : [cat]).map((it: string, idx: number) => (
                                                                        <li key={idx}>
                                    ...
                                    <div className="additional-info mb-60">
                                        ...
                                        {dynamicAdditionalInfo ? (
                                            <ul className="items-list two">
                                                {dynamicAdditionalInfo.map((info: any, idx: number) => (
                                                    <li key={idx}>
                                    ...
                                    {/* Customer Review & Rating - commented as per requirement */}
                                    {/* <div className="mb-50">
                                                    ...
                                                    <div className="faq-wrap">
                                                        <div className="accordion accordion-flush">
                                                            {displayFaqs?.map((item: any, index: number) => {
                                                                const faqIdStr = String(index);
                                                                const isOpen = active === faqIdStr;
                        ...
                        <div className="lg:col-span-4 lg:col-start-9">
                            <HotelSidebar
                                hotel={hotel}
                                price={(hotel as any)?.pricePerNight}
                                originalPrice={(hotel as any)?.originalPricePerNight}
                                discountPercent={(hotel as any)?.discountPercent}
                                currency={(hotel as any)?.currency}
                            />
                        </div>
